...
lc_model_data <- df_combined %>%
  mutate(
    ...
    nh4 = ifelse(nh4 == "LOD", -99, as.numeric(nh4)),
    nitrate = ifelse(nitrate == "LOD", -99, as.numeric(nitrate)),
    mg = ifelse(mg == "LOD", -99, as.numeric(mg)),
    p = ifelse(p == "LOD", -99, as.numeric(p)),
    ec = ifelse(ec == "LOD", -99, as.numeric(ec)),
    p_h = ifelse(p_h == "LOD", -99, as.numeric(p_h)),
...
library(brms)
fit <- brm(
  formula = bf(nitrate | cens(nitrate_censored) ~ date * treatment * study_year + (1 | site_block_id) + (1 | trt_site_block_id)),
  data = lc_model_data,
  family = Gamma(link = "log"),
  chains = 4,
  warmup = 1000,
...
summary(fit)
neff_ratio(fit)
plot(fit, type = "trace", nvariables = 2)
...
conditional_effects(fit, effects = "treatment")
conditional_effects(fit, effects = "date:treatment")


emm_treatments <- emmeans(fit, ~ treatment)
emm_treatments
...
saveRDS(fit, file = "brms_nitrate_model.rds")
