...
NOTE:
- try left censored
- email abt negative values in data
...
```{r}
# This create the same model as the code chunk after, just using a different package
# fit_lmer <- lmer(
# 
#   ec ~
...
# sim_res_gamma <- DHARMa::simulateResiduals(fit_glmer_gamma_ec, plot = TRUE)
...
emmip(fit_lme_ph, treatment ~ date, type = "response", CIs = T) + theme_bw()
```
...
emmip(fit_p, treatment ~ date, type = "response", CIs = T) + theme_bw()
```
...
lc_model_data <- df_combined %>%
  mutate(
    ...
    nh4 = ifelse(nh4 == "LOD", NA, as.numeric(nh4)),
    nitrate = ifelse(nitrate == "LOD", NA, as.numeric(nitrate)),
    mg = ifelse(mg == "LOD", NA, as.numeric(mg)),
    p = ifelse(p == "LOD", NA, as.numeric(p)),
    ec = ifelse(ec == "LOD", NA, as.numeric(ec)),
    p_h = ifelse(p_h == "LOD", NA, as.numeric(p_h)),
...
fit_p <- survreg(
  Surv(p, p_censored == 0, type = "left") ~ date * treatment,
  data = lc_model_data,
  dist = "gaussian"  # Or "lognormal" if needed
)

summary(fit_p)

# Predict expected values (ignoring censoring)
predicted <- predict(fit_p, type = "response")

# Compare with observed (non-censored) data
observed <- lc_model_data$p[lc_model_data$p_censored == 0]

plot(predicted[lc_model_data$p_censored == 0], observed,
     xlab = "Predicted", ylab = "Observed",
     main = "Predicted vs. Observed (Non-Censored)")
abline(0, 1, col = "red")  # Points should cluster around the line
```
