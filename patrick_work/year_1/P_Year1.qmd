...
Checking Site By Treatement

```{r}
library(lmerTest)
Sitemodel<- lmer(log(P+.001) ~ Treatment * Site + (1 |Site: Block), data = DF)

anova(Sitemodel, type = 3)
shapiro.test(resid(Sitemodel))

qqnorm(resid(Sitemodel))
qqline(resid(Sitemodel),col="red",lwd=2)

df_res <- data.frame(
  Fitted = fitted(Sitemodel),
  Residuals = resid(Sitemodel)
)
...
P_model <- glmmTMB((sqrt(P)) ~ Treatment * Date_scaled * Site + (1 | Site:Block),
                     ziformula = ~1,
...
em <- emmeans(P_model, pairwise ~ Treatment, adjust = "tukey", type = "response")
...
P_model <- lmer(log(P) ~ Treatment * Date_scaled * Site + (1 |Site:Block), data = DF)
anova(P_model, type = 3)
...
```{r}
hist(DF$P,breaks=200)
hist(log(DF$P+.001),breaks=100)
```
