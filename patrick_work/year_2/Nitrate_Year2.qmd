...
```{r}
NitTime<-ggplot(df.summaryNitrate, aes(Date, Nitratemean, color = Treatment)) +
...
NitSum<-ggplot(df.summaryNitrate, aes(x = Treatment, y = Nitratemean, fill = Treatment)) + 
  ...
  ggsave("NitTime.png", plot = NitTime, width = 8, height = 5, dpi = 300)
  ggsave("NitSum.png", plot = NitSum, width = 8, height = 5, dpi = 300)
...
NitFlux<-ggplot(df_integral, aes(Treatment, Total_Nitrate)) +
  ...
  ggsave("NitFlux.png", plot = NitFlux, width = 8, height = 5, dpi = 300)
...
Checking Stats at each sampling Date
